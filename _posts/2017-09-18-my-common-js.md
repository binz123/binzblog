...
description: 常用JS代码库，便于以后开发
keywords: javascript, 库, 杂项
---

## 说明

转前端半年，虽然时间不长，但是也积累了不少经验，但是发现开发过程中有许多经常需要用到的函数，每次重新写花费的时间跟精力太长，而且没有任何营养，索性开始写自己常用的库，便于以后开发，也可以方便后人使用。
最后也介绍一下如何封装自己的库，不会与其他库冲突，调用的时候，引入再直接调用即可了。
...
//6 blog在strTest中出现的次数

```

## 2.数组操作

**数组去重:**其实数组去重的方法有许多，像ES6中就有Set用于去重，这里将Set以及ES5的方法都列出来。

```javascript
//Set方法
function removeReapeatBySet(arr){
  let set = new Set(arr);
  //ES6中Array新增静态方法from,将类似数组的对象转换为数组
...
原生：html(document.getElementById('xxx'),'hello world')
//获取对象内容
jquery：$('#xxx').html();
原生：html(document.getElementById('xxx'))
